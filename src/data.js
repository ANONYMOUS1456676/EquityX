...
export function generateHistory(basePrice) {
  const history = [];
  const now = Date.now();
  for (let i = MAX_HISTORY; i >= 0; i--) {
    ...
    history.push({
      timestamp,
      price: Math.max(10, basePrice + variation),
...
export function updateStock(stocks, name, newData) {
  const next = { ...stocks };
  const current = next[name] || { history: [] };
  ...
  next[name] = { ...current, ...newData, history };
  return next;
}

export function simulateUpdate(stocks, name) {
  const current = stocks[name];
  if (!current) return stocks;
  ...
  return updateStock(stocks, name, {
    price: newPrice,
    change,
    changePercent: ((change / current.price) * 100).toFixed(2) + '%',
    volume: Math.max(0, current.volume + Math.floor((Math.random() - 0.5) * 100000)),
    lastUpdated: new Date()
  });
}
